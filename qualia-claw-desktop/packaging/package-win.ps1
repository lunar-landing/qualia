# Qualia Claw Desktop - Windows 打包脚本（jpackage app-image）
#
# 前置要求：
...
#
# 产物：qualia-claw-desktop\target\dist\<version>\Qualia Claw\Qualia Claw.exe（免安装目录，可整体 zip 分发）

$ErrorActionPreference = "Stop"

$ProjectRoot = Resolve-Path "$PSScriptRoot\..\.."
$ModuleDir = Join-Path $ProjectRoot "qualia-claw-desktop"
...
# 先安装 core/claw 到本地仓库，再打包 desktop 模块的 shaded jar
& mvn -q -pl qualia-claw-desktop -am clean package -DskipTests
if ($LASTEXITCODE -ne 0) { throw "Maven 构建失败" }

# 定位 shaded jar（shade 插件覆盖原 jar，同名输出）
$Jar = Get-ChildItem -Path $TargetDir -Filter "qualia-claw-desktop-*.jar" |
...
& $JPackage `
    ...
    --input $InputDir `
    --main-jar $Jar.Name `
    --main-class cn.lunarlanding.qualia.claw.desktop.ClawDesktopLauncher `
    --dest $DistDir `
...
Write-Host ""
Write-Host "==> 完成：$DistDir\Qualia Claw\Qualia Claw.exe"
Write-Host "   可将整个 'Qualia Claw' 目录打包 zip 分发（免安装）。"
