 ...
 *
 * 职责：
 *   1. 拉取当前智能体的 /api/agents/{agentId}/chat/stats/tokens?days=30，按当期最大值动态分 4 档渲染 10×3 方格矩阵
 ...
 *
 * 对外 API：
 *   TokenHeatmap.init(containerEl, agentIdGetter)   绑定容器并注入骨架，随即拉数据渲染（agentIdGetter 返回当前智能体 id）
...
window.TokenHeatmap = (function () {
    ...
    let container = null;
    /** 当前智能体 id 提供者（Claw 下统计接口挂在智能体维度） */
    let agentIdGetter = null;

    function agentId() {
        return agentIdGetter ? agentIdGetter() : '';
    }

    function statsUrl(days) {
        return `/api/agents/${encodeURIComponent(agentId())}/chat/stats/tokens?days=${days}`;
    }
    ...
    async function refresh() {
        ...
        try {
            const resp = await fetch(statsUrl(30));
            if (!resp.ok) return;
    ...
    function init(containerEl, idGetter) {
        container = containerEl;
        agentIdGetter = idGetter || null;
        if (!container) return;
    ...
    async function renderDetail() {
        ...
        let stats = [];
        try {
            const resp = await fetch(statsUrl(currentDays));
